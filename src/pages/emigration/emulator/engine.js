...
export const CONNECTIONS_CATALOG = [
  ...
  { name: 'Cookies for Neighbor from Destination', cost: 2, type: 'connection' },
  { name: 'Video Chat with Person from Destination', cost: 2, type: 'connection' },
...
export const LIFE_CARDS_CATALOG = [
  // Friendship pack
  { title: 'Stellar Reputation', pack: 'Friendship', keep: 'May Keep', type: 'life' },
  { title: 'Rummage Sale', pack: 'Friendship', keep: 'Instant', type: 'life' },
  { title: 'Island Paradise', pack: 'Friendship', keep: 'Instant', type: 'life' },
  { title: 'Swap Wallets', pack: 'Friendship', keep: 'Instant', type: 'life' },
  // High Society pack
  { title: 'VIP', pack: 'High Society', keep: 'Instant', type: 'life' },
  { title: 'Fancy Clothes', pack: 'High Society', keep: 'May Keep', type: 'life' },
  { title: 'Social Butterfly', pack: 'High Society', keep: 'Instant', type: 'life' },
  { title: 'Identical Twin', pack: 'High Society', keep: 'Instant', type: 'life' },
  // Downtown pack
  ...
  // Emergency pack
  { title: 'Trousers Fall Down', pack: 'Emergency', keep: 'Instant', type: 'life' },
  { title: 'Keep Calm', pack: 'Emergency', keep: 'Must Keep', type: 'life' },
  { title: 'Life Coach', pack: 'Emergency', keep: 'Instant', type: 'life' },
  { title: 'Shredder Accident', pack: 'Emergency', keep: 'Instant', type: 'life' },
  // Vacation pack
  ...
  { title: 'Pandemic / Economic Stimulus', pack: 'News', keep: 'Instant', type: 'life' },
  { title: 'Pandemic / Economic Stimulus', pack: 'News', keep: 'Instant', type: 'life' },
...
export const PACKS_LIST = [
  'Friendship', 'High Society', 'Downtown', 'Emergency',
  'Vacation', 'News', 'Charity', 'Trade', 'Sports',
];
...
export default class EmigrationEngine {
   ...
   */
  constructor({ mode, players: playersSetup, rollOverride = null, onLog = null, onStateChange = null }) {
    this.mode = mode;
  ...
  _setupPlayersAndDeck(playersSetup) {
    ...
    this.log(
      ...
      `${packsCount} Life Packs, ${paydaysCount} Paydays.`, 'system'
    );

    // Build the deck
    const selectedDocs = shuffleArray([...DOCUMENTS_CATALOG]).slice(0, docConnEach);
    const selectedConns = shuffleArray([...CONNECTIONS_CATALOG]).slice(0, docConnEach);
    const selectedPacks = shuffleArray([...PACKS_LIST]).slice(0, packsCount);
    this.log(`Life Card packs: ${selectedPacks.join(', ')}`, 'system');

    const selectedLifeCards = LIFE_CARDS_CATALOG
      .filter(c => selectedPacks.includes(c.pack))
      .map(c => ({ ...c }));

    const paydays = Array.from({ length: paydaysCount }, () => ({
      name: 'Payday', type: 'payday', cost: 0,
    }));
    ...
    mainDeck.splice(0, 2);
    this.log(`Removed 2 cards face-down. Deck: ${mainDeck.length} cards.`, 'system');
    ...
    playersSetup.forEach((setup, idx) => {
      ...
      this.log(
        ...
        `starts with $${player.money}.`, 'system'
      );
    });
  }
    ...
    for (let i = 0; i < 14; i++) {
      ...
      if (slot && !slot.faceUp && !this.isCardCovered(player, i)) {
        ...
        this.log(
          ...
          `(slot ${i}) is uncovered and flipped face-up.`, 'system'
        );
      }
    }
  }
  ...
  _onCardDiscarded(discardingPlayer, card) {
    ...
    if (discardingPlayer.stash.lifeCards.some(lc => lc.title === 'Blacklisted')) {
  ...
  _onPlayerGainDocument(player) {
    const idx = player.stash.lifeCards.findIndex(lc => lc.title === 'Penalty');
    if (idx !== -1) {
  ...
  _onPlayerGainConnection(player) {
    for (const p of this.players) {
      if (p.id !== player.id) {
        const idx = p.stash.lifeCards.findIndex(lc => lc.title === 'Star Power');
        if (idx !== -1) {
          p.money += 1;
  ...
  _onPlayerGainLifeCard(player) {
    const idx = player.stash.lifeCards.findIndex(lc => lc.title === 'Underdog');
    if (idx !== -1) {
  ...
  checkTicketPassportBonus(player) {
    if (!player.ticketPassportBonusClaimed) {
      if (player.stash.tickets >= 1 && player.stash.passports >= 1) {
        ...
        this.log(`${player.name} gains +1 Assurance (Ticket + Passport bonus).`, 'system');
      }
    } else {
  ...
  getEffectiveCost(player, card) {
    ...
    if (card.type === 'document' && player.stash.lifeCards.some(lc => lc.title === 'Fancy Clothes')) {
      cost = Math.max(0, cost - 1);
    }
    if (card.type === 'connection' && player.stash.lifeCards.some(lc => lc.title === 'Stellar Reputation')) {
      cost = Math.max(0, cost - 1);
    }
    return cost;
  }
  ...
  canPerformAnyRequiredAction(player) {
    ...
    if (this.publicServices.tickets > 0 && player.stash.connections.length >= 1 && player.money >= 2) return true;
    if (this.publicServices.passports > 0 && player.stash.documents.length >= 1 && player.money >= 2) return true;

    // 3. Steal from pool (free)
    if (this.publicServices.tickets > 0 && player.stash.connections.length >= 1) return true;
    if (this.publicServices.passports > 0 && player.stash.documents.length >= 1) return true;

    // 4. Reclaim from opponent stash
    for (const p of this.players) {
      if (p.id !== player.id) {
        const cost = 2 + player.accessFee;
        if (player.money >= cost && (p.stash.tickets > 1 || p.stash.passports > 1)) return true;
      }
    }

    // 5. Discard from own stash (free, earns 2)
    if (player.stash.documents.length > 0 || player.stash.connections.length > 0) return true;
    ...
    // 6. Apply for College
    if (player.payRaises < MAX_PAY_RAISES && !player.inCollege) {
  ...
  getValidActions(player) {
    ...
    // Optional: Graduate
    actions.push({ type: 'graduate', label: 'Graduate (D6)', optional: true, enabled: p.inCollege });

    // Optional: Sell
    const canSell = p.stash.documents.length > 0 || p.stash.connections.length > 0;
    actions.push({ type: 'sell', label: 'Sell Stash Card', optional: true, enabled: canSell });
      ...
      if (p.money >= fee) {
        for (let i = 0; i < 14; i++) {
          if (this.isCardAvailable(op, i)) {
            ...
            if (t === 'payday' || t === 'life') { canActivate = true; break; }
          }
        }
      }
      if (canActivate) break;
    }
    actions.push({ type: 'activate', label: 'Activate Card', optional: false, enabled: canActivate });
    ...
    for (const op of this.players) {
      ...
      for (let i = 0; i < 14; i++) {
        if (this.isCardAvailable(op, i)) {
          ...
          if (card.type === 'document' || card.type === 'connection') {
            if (p.money >= this.getEffectiveCost(p, card) + fee) { canBuy = true; break; }
          }
        }
      }
      if (canBuy) break;
    }
    if (this.publicServices.tickets > 0 && p.stash.connections.length >= 1 && p.money >= 2) canBuy = true;
    if (this.publicServices.passports > 0 && p.stash.documents.length >= 1 && p.money >= 2) canBuy = true;
    actions.push({ type: 'buy', label: 'Buy Card', optional: false, enabled: canBuy });

    // Required: Steal
    const canStealT = this.publicServices.tickets > 0 && p.stash.connections.length >= 1;
    const canStealP = this.publicServices.passports > 0 && p.stash.documents.length >= 1;
    actions.push({ type: 'steal', label: 'Steal (Skip Turn)', optional: false, enabled: canStealT || canStealP });

    // Required: Reclaim
    let canReclaim = false;
    for (const op of this.players) {
      if (op.id !== p.id && p.money >= 2 + p.accessFee && (op.stash.tickets > 1 || op.stash.passports > 1)) {
        canReclaim = true; break;
      }
    }
    actions.push({ type: 'reclaim', label: 'Reclaim Card', optional: false, enabled: canReclaim });

    // Required: Discard
    let canDiscard = p.stash.documents.length > 0 || p.stash.connections.length > 0;
    if (!canDiscard) {
      for (const op of this.players) {
        const fee = op.id === p.id ? 0 : p.accessFee;
        if (p.money >= fee) {
          for (let i = 0; i < 14; i++) {
            if (this.isCardAvailable(op, i)) {
              const t = op.layout[i].card.type;
              if (t === 'document' || t === 'connection') { canDiscard = true; break; }
            }
          }
        }
        if (canDiscard) break;
    ...
    actions.push({ type: 'discard', label: 'Discard Card', optional: false, enabled: canDiscard });

    // Required: Apply for College
    let canApply = false;
    if (p.payRaises < MAX_PAY_RAISES && !p.inCollege) {
      const minTuition = Math.floor(p.collegeFund / 2) + 1;
      if (p.money >= minTuition) canApply = true;
    }
    actions.push({ type: 'applyCollege', label: 'Apply for College', optional: false, enabled: canApply });
  ...
  advanceTurn() {
    ...
    if (this._identicalTwinExtraTurn) {
      this._identicalTwinExtraTurn = false;
      this.log(`${this.players[this.currentPlayerIdx].name} takes an extra turn (Identical Twin).`, 'system');
      this._notify();
      return;
    }
    ...
    if (!this.canPerformAnyRequiredAction(next)) {
      ...
      this.log(
        ...
        `Consecutive: ${this.consecutiveForfeits}/${this.players.length}`, 'error'
      );
  ...
  checkPhase2Trigger() {
    const poolEmpty = this.publicServices.tickets === 0 && this.publicServices.passports === 0;
  ...
  triggerPhase2() {
    ...
    for (const player of this.players) {
      ...
      this.log(
        ...
        `Assurance: ${player.assurance}`, 'system'
      );
    }
  ...
  selectLane(laneIdx) {
    ...
    this.log(`${player.name} selected ${lane.name}: token value = ${tokenVal}`, 'action');
    ...
    if (!hasTicket || !hasPassport) {
      ...
      this.log(
        ...
        `Remaining: ${player.assurance}`, 'action'
      );
    } else {
      player.crossedSuccessfully = false;
      this.log(
    ...
    if (this.activeCrossingIdx >= this.players.length) {
      ...
      this.log(`${this.players[this.activeCrossingIdx].name} selects a lane...`, 'system');
    }
    this._notify();
  }
  ...
  _endGame() {
    ...
    if (this.mode === 'cooperative') {
      ...
      if (allCrossed) {
        this.gameResult = { winner: null, message: 'COOPERATIVE WIN! Everyone crossed!', playerResults };
        this.log(this.gameResult.message, 'system');
      } else {
        const failed = this.players.filter(p => !p.crossedSuccessfully).map(p => p.name);
        this.gameResult = {
          winner: null,
  ...
  executeOptionalAction(type, params = {}) {
    ...
    switch (type) {
      case 'graduate': {
        if (!player.inCollege) { this.log('Not in college.', 'error'); return; }
        ...
        if (roll <= 3) {
          ...
          this.log(
            ...
            `salary +$${raiseAmount} → $${player.salary}/payday.`, 'action'
          );
        } else {
          this.log(`${player.name} failed exams (rolled ${roll}). Remains in college.`, 'error');
        }
        this._notify();
        break;
      }
      ...
      case 'sell': {
        ...
        const arr = stashType === 'document' ? player.stash.documents : player.stash.connections;
        if (stashIdx < 0 || stashIdx >= arr.length) { this.log('Invalid stash index.', 'error'); return; }
  ...
  executeRequiredAction(type, params = {}) {
    ...
    switch (type) {
      case 'activate': return this._doActivate(player, params);
      case 'buy': return this._doBuy(player, params);
      case 'buyPool': return this._doBuyPool(player, params);
      case 'steal': return this._doSteal(player, params);
      case 'reclaim': return this._doReclaim(player, params);
      case 'discard': return this._doDiscard(player, params);
      case 'applyCollege': return this._doApplyCollege(player);
    }
  }
  ...
  _doActivate(player, { targetPlayerIdx, slotIdx }) {
    ...
    if (!slot || !this.isCardAvailable(target, slotIdx)) {
      this.log('Card is not available.', 'error'); return;
    }
    if (slot.card.type !== 'payday' && slot.card.type !== 'life') {
      this.log('Can only activate Payday or Life cards.', 'error'); return;
    }

    const fee = target.id === player.id ? 0 : player.accessFee;
    if (player.money < fee) {
      this.log(`Cannot afford access fee ($${fee}).`, 'error'); return;
    }
    ...
    if (slot.card.type === 'life') {
      ...
      if (player.stash.lifeCards.some(lc => lc.title === 'Keep Calm')) {
        this._setPendingChoice({
          ...
          resolve: (val) => {
            if (val === 'use') {
              const kcIdx = player.stash.lifeCards.findIndex(lc => lc.title === 'Keep Calm');
              ...
              this.log(`${player.name} used Keep Calm to discard ${removed.card.title}.`, 'action');
      ...
      if (target.id !== player.id && target.stash.lifeCards.some(lc => lc.title === 'Persuasion')) {
    ...
    this._setPendingChoice({
      ...
      resolve: (val) => {
        ...
        this._setPendingChoice({
          ...
          options: [
            { text: `Accept Persuasion (pay $${baseFee} fee)`, value: 'accept' },
            { text: `Decline (pay $${baseFee * 2} double fee)`, value: 'decline' },
          ],
          resolve: (buyerVal) => {
            if (buyerVal === 'accept') {
              ...
              const persIdx = target.stash.lifeCards.findIndex(lc => lc.title === 'Persuasion');
              const [persCard] = target.stash.lifeCards.splice(persIdx, 1);
              player.stash.lifeCards.push(persCard);
              this.log(`${player.name} accepted Persuasion from ${target.name}.`, 'action');
              ...
              this.log(`${player.name} declined Persuasion. Paying double fee ($${doubleFee}).`, 'action');
              callback(doubleFee);
            }
          },
        });
  ...
  _payAccessFee(player, target, fee) {
    if (fee > 0 && target.id !== player.id) {
      ...
      this.log(`${player.name} paid $${fee} access fee to ${target.name}.`, 'system');
    }
  }
  ...
  _resolveLifeCardKeep(player, card, done) {
    switch (card.title) {
      case 'Stellar Reputation':
        ...
        this.log(`${player.name} keeps Stellar Reputation (Connections cost -$1).`, 'action');
        done();
        break;
      case 'Fancy Clothes':
        player.stash.lifeCards.push({ ...card, keep: 'Must Keep' });
        this.log(`${player.name} keeps Fancy Clothes (Documents cost -$1).`, 'action');
        done();
        break;
      case 'Insider':
        player.stash.lifeCards.push({ ...card, keep: 'Must Keep' });
  ...
  _resolveLifeCardEffect(player, card, done) {
    ...
    switch (title) {
      ...
      case 'Rummage Sale': {
        ...
        if (discDocs.length > 0) {
          this._setPendingChoice({
            ...
            options: [
              ...
              ...discDocs.map((c, i) => ({ text: `Take: ${c.name}`, value: `doc-${i}` })),
            ],
            resolve: (val) => {
              if (val === 'money') {
                player.money += 3;
                ...
                const discIdx = this.discardPile.findIndex(c => c.type === 'document');
                if (discIdx !== -1) {
                  ...
                  for (let i = 0; i < this.discardPile.length; i++) {
                    if (this.discardPile[i].type === 'document') {
                      if (count === docIndexInDiscard) {
                        ...
                        this.log(`${player.name} takes ${taken.name} from discard.`, 'action');
        ...
        player.money += 3;
        this.log(`${player.name} gains $3 (no documents in discard).`, 'action');
        done(); break;
      }
      ...
      case 'Island Paradise':
        ...
        {
          const minDocs = Math.min(...this.players.map(p => p.stash.documents.length));
          for (const p of this.players) {
            if (p.stash.documents.length === minDocs) {
              p.money += 1;
              this.log(`${p.name} (fewest Documents: ${minDocs}) gains $1.`, 'system');
            }
          }
        }
        done(); break;
      ...
      case 'Swap Wallets':
        this._promptTargetPlayer(player, 'Swap Wallets: choose a player', (targetId) => {
          const target = this.players[targetId];
          const temp = player.money;
          player.money = target.money;
          target.money = temp;
          this.log(`${player.name} swapped wallets with ${target.name}.`, 'action');
          done();
        });
        break;
      ...
      case 'VIP': {
        ...
        player.money += bonus;
        this.log(`VIP: richest player has $${maxM}. ${player.name} gains $${bonus}.`, 'action');
        done(); break;
      }
      ...
      case 'Social Butterfly':
        this._promptTargetPlayer(player, 'Social Butterfly: choose target', (targetId) => {
          const target = this.players[targetId];
          const options = [];
          if (target.stash.connections.length > 0) {
            options.push({ text: `Take 1 Connection from ${target.name}`, value: 'conn' });
          }
          options.push({ text: `Take up to $3 from ${target.name}`, value: 'money' });

          if (options.length === 1 && options[0].value === 'money') {
            const stolen = Math.min(3, target.money);
            target.money -= stolen;
            player.money += stolen;
            this.log(`${player.name} takes $${stolen} from ${target.name}.`, 'action');
            done();
            return;
          }
              ...
              if (val === 'conn') {
                this._setPendingChoice({
                  ...
                  options: target.stash.connections.map((c, i) => ({
                    text: `${c.name} ($${c.cost})`, value: String(i),
                  })),
                  resolve: (connIdx) => {
                    const [taken] = target.stash.connections.splice(parseInt(connIdx), 1);
                    player.stash.connections.push(taken);
                    this.log(`${player.name} takes ${taken.name} from ${target.name}.`, 'action');
      ...
      case 'Reward':
        ...
        for (const p of this.players) {
          if (p.id !== player.id) {
            ...
            p.money -= amt;
            player.money += amt;
            if (amt > 0) this.log(`${player.name} takes $1 from ${p.name}.`, 'action');
          }
        }
        done(); break;
      ...
      case 'Suspect':
        ...
        if (player.stash.documents.length > 0 || player.stash.connections.length > 0) {
          const opts = [];
          if (player.stash.documents.length > 0) opts.push({ text: 'Lose 1 Document', value: 'doc' });
          if (player.stash.connections.length > 0) opts.push({ text: 'Lose 1 Connection', value: 'conn' });
          this._setPendingChoice({
            id: 'suspect-penalty',
            title: 'Suspect: lose a Document or Connection',
            options: opts,
            resolve: (val) => {
              const arr = val === 'doc' ? player.stash.documents : player.stash.connections;
              if (arr.length === 1) {
                ...
                this._setPendingChoice({
                  ...
                  options: arr.map((c, i) => ({ text: c.name, value: String(i) })),
                  resolve: (idx) => {
          ...
          this._setPendingChoice({
            ...
            resolve: (val) => {
              if (val === 'money') {
                ...
                this._promptSelectCard(player.stash.documents, 'Document to lose', (idx) => {
      ...
      case 'Shredder Accident':
        if (player.stash.documents.length > 0) {
          this._promptSelectCard(player.stash.documents, 'Document to lose (Shredder)', (idx) => {
            const [disc] = player.stash.documents.splice(idx, 1);
            this.discardPile.push(disc);
            this.log(`${player.name} loses ${disc.name} (Shredder).`, 'action');
            this._onCardDiscarded(player, disc);
            done();
          });
          return;
      ...
      case 'Camping':
        ...
        this.log(`${player.name} gains $1.`, 'action');
        {
          const minConns = Math.min(...this.players.map(p => p.stash.connections.length));
          for (const p of this.players) {
            if (p.stash.connections.length === minConns) {
              p.money += 1;
              this.log(`${p.name} (fewest Connections: ${minConns}) gains $1.`, 'system');
            }
          }
        }
        done(); break;
      ...
      case 'FOMO':
        ...
        this._setPendingChoice({
          ...
          options: [
            ...
            ...this.players.filter(p => p.id !== player.id).map(p => ({
              text: `Trade with ${p.name} (${p.destination})`, value: String(p.id),
            })),
          ],
          resolve: (val) => {
            if (val !== 'skip') {
              ...
              this.log(`${player.name} and ${target.name} swapped destinations.`, 'action');
            }
            done();
          },
        });
      ...
      case 'Nostalgia': {
        ...
        if (discLife.length > 0) {
          this._setPendingChoice({
            ...
            options: [
              ...
              ...discLife.map((c, i) => ({ text: `Replay: ${c.title}`, value: `life-${i}` })),
            ],
            resolve: (val) => {
              if (val === 'money') {
                player.money += 2;
                ...
                for (let i = 0; i < this.discardPile.length; i++) {
                  if (this.discardPile[i].type === 'life') {
                    if (count === lifeIdx) {
                      const [taken] = this.discardPile.splice(i, 1);
                      this.log(`${player.name} replays ${taken.title}.`, 'action');
        ...
        player.money += 2;
        this.log(`${player.name} gains $2 (no Life Cards in discard).`, 'action');
        done(); break;
      }

      case 'Lost & Found':
        this._promptTargetPlayer(player, 'Lost & Found: choose target', (targetId) => {
          const target = this.players[targetId];
          const opts = [];
          if (target.stash.documents.length > 0) {
            opts.push({ text: `Take 1 Document from ${target.name}`, value: 'doc' });
          }
            ...
            resolve: (val) => {
              if (val === 'doc') {
                this._setPendingChoice({
                  ...
                  options: target.stash.documents.map((c, i) => ({ text: c.name, value: String(i) })),
                  resolve: (idx) => {
                    const [taken] = target.stash.documents.splice(parseInt(idx), 1);
                    player.stash.documents.push(taken);
                    this.log(`${player.name} takes ${taken.name} from ${target.name}.`, 'action');
                    this._onPlayerGainDocument(player);
                    done();
      ...
      case 'Pandemic / Economic Stimulus': {
        ...
        if (this.pandemicStimulusCount % 2 === 1) {
          ...
          this.log(`Pandemic! Everyone loses $${roll}. (Activation #${this.pandemicStimulusCount})`, 'action');
        } else {
          // Even: everyone gains
          for (const p of this.players) {
            p.money += roll;
          }
          this.log(`Economic Stimulus! Everyone gains $${roll}. (Activation #${this.pandemicStimulusCount})`, 'action');
        }
        done(); break;
      }
          ...
          this._setPendingChoice({
            ...
            options: [
              ...
              ...player.stash.lifeCards.map((c, i) => ({
                text: `Discard: ${c.title}`, value: String(i),
              })),
            ],
            resolve: (val) => {
              if (val !== 'skip') {
      ...
      case 'Philanthropy': {
        ...
        while (idx !== player.id) {
          if (player.money > 0) {
            ...
            this.log(`${player.name} gives $1 to ${this.players[idx].name}.`, 'system');
          }
      ...
      case 'Productivity':
        ...
        this.log(`${player.name} gains $1, Access Fee → $${player.accessFee}.`, 'action');
        done(); break;

      case 'Tariffs':
        player.money = Math.max(0, player.money - 1);
        player.accessFee = Math.min(5, player.accessFee + 1);
        this.log(`${player.name} loses $1, Access Fee → $${player.accessFee}.`, 'action');
        done(); break;

      case 'Boost':
        this._promptTargetPlayer(player, 'Boost: choose player (gain half their Nationality fund)', (targetId) => {
          const target = this.players[targetId];
          const amt = Math.floor(target.collegeFund / 2);
          player.money += amt;
          this.log(`${player.name} gains $${amt} (half of ${target.name}'s fund: $${target.collegeFund}).`, 'action');
          done();
        });
        break;
      ...
      case 'Frontrunner':
        ...
        this.log(`${player.name} keeps Frontrunner ($1 placed on card).`, 'action');
        done(); break;
  ...
  _promptSelectCard(arr, title, callback) {
    if (arr.length === 1) { callback(0); return; }
    this._setPendingChoice({
      id: 'select-card',
      title,
      options: arr.map((c, i) => ({ text: c.name || c.title, value: String(i) })),
      resolve: (val) => callback(parseInt(val)),
    });
  }
  ...
  _doBuy(player, { targetPlayerIdx, slotIdx }) {
    ...
    const slot = target.layout[slotIdx];
    if (!slot || !this.isCardAvailable(target, slotIdx)) {
      this.log('Card not available.', 'error'); return;
    }
    const card = slot.card;
    if (card.type !== 'document' && card.type !== 'connection') {
      this.log('Can only buy Document/Connection from layout.', 'error'); return;
    }
  ...
  _finishBuy(player, target, slotIdx, cost, fee) {
    ...
    if (player.money < totalCost) {
      this.log(`Cannot afford $${totalCost}.`, 'error'); return;
    }
    ...
    if (card.type === 'document') {
      ...
      this.log(`${player.name} bought ${card.name} ($${cost}) from ${target.name}.`, 'action');
      this._onPlayerGainDocument(player);
    } else {
  ...
  _doBuyPool(player, { cardType }) {
    if (cardType === 'ticket') {
      if (this.publicServices.tickets <= 0) { this.log('No Tickets in pool.', 'error'); return; }
      if (player.stash.connections.length < 1) { this.log('Need ≥1 Connection.', 'error'); return; }
      if (player.money < 2) { this.log('Need $2.', 'error'); return; }
      player.money -= 2;
      this.publicServices.tickets--;
      player.stash.tickets++;
      this.log(`${player.name} bought a Ticket from the pool.`, 'action');
    } else {
      if (this.publicServices.passports <= 0) { this.log('No Passports in pool.', 'error'); return; }
      if (player.stash.documents.length < 1) { this.log('Need ≥1 Document.', 'error'); return; }
      if (player.money < 2) { this.log('Need $2.', 'error'); return; }
      player.money -= 2;
  ...
  _doSteal(player, { cardType }) {
    if (cardType === 'ticket') {
      if (this.publicServices.tickets <= 0 || player.stash.connections.length < 1) {
        this.log('Cannot steal a Ticket.', 'error'); return;
      }
      this.publicServices.tickets--;
      player.stash.tickets++;
      ...
      if (this.publicServices.passports <= 0 || player.stash.documents.length < 1) {
        this.log('Cannot steal a Passport.', 'error'); return;
      }
      this.publicServices.passports--;
      player.stash.passports++;
  ...
  _doReclaim(player, { targetPlayerIdx, cardType }) {
    const target = this.players[targetPlayerIdx];
    if (target.id === player.id) { this.log('Cannot reclaim from yourself.', 'error'); return; }

    const cost = 2 + player.accessFee;
    if (player.money < cost) { this.log(`Cannot afford $${cost}.`, 'error'); return; }

    if (cardType === 'ticket') {
      if (target.stash.tickets <= 1) { this.log(`${target.name} has ≤1 Ticket.`, 'error'); return; }
      player.money -= cost;
      target.money += cost;
      player.accessFee = Math.min(5, player.accessFee + 1);
      target.stash.tickets--;
      player.stash.tickets++;
      this.log(`${player.name} reclaimed a Ticket from ${target.name} ($${cost}).`, 'action');
    } else {
      if (target.stash.passports <= 1) { this.log(`${target.name} has ≤1 Passport.`, 'error'); return; }
      player.money -= cost;
      target.money += cost;
      player.accessFee = Math.min(5, player.accessFee + 1);
      target.stash.passports--;
      player.stash.passports++;
      this.log(`${player.name} reclaimed a Passport from ${target.name} ($${cost}).`, 'action');
    }
  ...
  _doDiscard(player, { source, targetPlayerIdx, slotIdx, stashType, stashIdx }) {
    if (source === 'stash') {
      if (targetPlayerIdx !== undefined && targetPlayerIdx !== player.id) {
        this.log('Cannot discard from another player\'s stash.', 'error'); return;
      }
      const arr = stashType === 'document' ? player.stash.documents : player.stash.connections;
      if (stashIdx < 0 || stashIdx >= arr.length) { this.log('Invalid index.', 'error'); return; }
      const [disc] = arr.splice(stashIdx, 1);
      this.discardPile.push(disc);
      player.money += 2;
      this.log(`${player.name} discarded ${disc.name} from stash, gains $2.`, 'action');
      this._onCardDiscarded(player, disc);
      this.advanceTurn();
    } else {
      // From layout
      ...
      if (slot.card.type !== 'document' && slot.card.type !== 'connection') {
        this.log('Can only discard Document/Connection.', 'error'); return;
      }

      const fee = target.id === player.id ? 0 : player.accessFee;
      if (player.money < fee) { this.log(`Cannot afford fee ($${fee}).`, 'error'); return; }

      this._payAccessFee(player, target, fee);
      const [removed] = target.layout.splice(slotIdx, 1, null);
      this.discardPile.push(removed.card);
      player.money += 2;
      this.log(`${player.name} discarded ${removed.card.name} from ${target.name}'s layout, gains $2.`, 'action');
  ...
  _doApplyCollege(player) {
    if (player.payRaises >= MAX_PAY_RAISES) { this.log('Career maxed (2/2 raises).', 'error'); return; }
    if (player.inCollege) { this.log('Already in college.', 'error'); return; }

    const minTuition = Math.floor(player.collegeFund / 2) + 1;
    if (player.money < minTuition) {
      this.log(`Cannot afford min tuition ($${minTuition}).`, 'error'); return;
    }
    ...
    this.log(`${player.name} applies for college. Roll: ${roll}, Tuition: $${tuition}`, 'action');

    if (player.money >= tuition) {
      player.money -= tuition;
      player.inCollege = true;
      this.log(`${player.name} paid $${tuition} and is now in college.`, 'action');
      this.advanceTurn();
    } else {
      player.money = Math.max(0, player.money - 1);
      ...
      this.log(
        ...
        `Must choose a different action.`, 'error'
      );
      this._notify();
    }
  }
...
export function runTests() {
  ...
  [2, 3, 4, 5, 6].forEach(P => {
    try {
      ...
      assert(eng.players[0].layout.filter(s => s !== null).length === 14, `[${P}P] 14 layout cards dealt`);
    } catch (e) {
  ...
  try {
    const bosnia = DESTINATIONS.find(d => d.name === 'Bosnia and Herzegovina');
  ...
  try {
    ...
    assert(!eng.canPerformAnyRequiredAction(p), 'Forfeit detected: no money, no cards');

    p.money = 2;
    assert(eng.canPerformAnyRequiredAction(p), 'Can apply for college with $2 (Bosnian min tuition)');
  } catch (e) {
  ...
  try {
    assert(NATIONALITY_TO_COUNTRY['Bosnian'] === 'Bosnia and Herzegovina', 'Bosnian → Bosnia');
    assert(NATIONALITY_TO_COUNTRY['American'] === 'United States of America', 'American → USA');
    assert(NATIONALITY_TO_COUNTRY['Congolese'] === 'Democratic Republic of Congo', 'Congolese → DRC');
  } catch (e) {
  ...
  try {
    ...
    assert(l1sorted[0] === 6 && l1sorted[1] === 7 && l1sorted[2] === 7, 'Lane 1 tokens = [6,7,7]');
  } catch (e) {
