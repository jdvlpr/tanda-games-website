<script>
  let { engine, snapshot, currentPlayer, actions, onaction, onselectlane, selectionText } = $props();
...
<div class="bg-emi-bg-panel rounded-xl p-5 flex flex-col gap-5 h-full max-h-[calc(100vh-40px)] overflow-hidden font-emi-ui text-slate-50 border border-white/10 backdrop-blur-md">
  {#if engine && snapshot}
